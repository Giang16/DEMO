...
public interface DongQuyRepository extends JpaRepository<DongQuy, Integer> {
    ...
    @Query("SELECT concat(nk.hovatendem,' ', nk.ten), dp.money FROM DongQuy dp join HoGiaDinh hgd on hgd.fid = dp.fid join NhanKhau nk on nk.cccd = hgd.cccdchuho WHERE dp.quyid = :phiid ORDER BY nk.ten ASC")
    List<String> findHoGiaDinhDaDongQuyOrderByNameASC(@Param("phiid") Integer phiid);

    @Query("SELECT concat(nk.hovatendem,' ', nk.ten), dp.money FROM DongQuy dp join HoGiaDinh hgd on hgd.fid = dp.fid join NhanKhau nk on nk.cccd = hgd.cccdchuho WHERE dp.quyid = :phiid ORDER BY nk.ten DESC")
    List<String> findHoGiaDinhDaDongQuyOrderByNameDESC(@Param("phiid") Integer phiid);

    @Query("SELECT concat(nk.hovatendem,' ', nk.ten), dp.money FROM DongQuy dp join HoGiaDinh hgd on hgd.fid = dp.fid join NhanKhau nk on nk.cccd = hgd.cccdchuho WHERE dp.quyid = :phiid ORDER BY dp.money ASC")
    List<String> findHoGiaDinhDaDongQuyOrderByMoneyASC(@Param("phiid") Integer phiid);

    @Query("SELECT concat(nk.hovatendem,' ', nk.ten), dp.money FROM DongQuy dp join HoGiaDinh hgd on hgd.fid = dp.fid join NhanKhau nk on nk.cccd = hgd.cccdchuho WHERE dp.quyid = :phiid ORDER BY dp.money DESC")
